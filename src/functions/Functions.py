...
class Functions(Inicializar):
    ...
    def abrir_navegador(self, URL=Inicializar.URL, navegador=Inicializar.NAVEGADOR):
        ...
        if navegador == ("CHROME"):
            ...
            self.driver = webdriver.Chrome(chrome_options=options,executable_path = Inicializar.basedir + "/drivers/chromedriver")
    ...
    def clear_key_text(self, entity):
    ...
    def send_key_num(self, entity, text):
        #Functions.get_elements(self, entity).send_keys(text)
        Functions.write_number(self, entity, text)


    #Crear Step
    def send_especific_keys(self, entity, key):
        if key == '0':
            Functions.get_elements(self, entity).send_keys(Keys.NUMPAD0)
        if key == '1':
            Functions.get_elements(self, entity).send_keys(Keys.NUMPAD1)
        if key == '9':
            Functions.get_elements(self, entity).send_keys(Keys.NUMPAD9)
        if key == '3':
            Functions.get_elements(self, entity).send_keys(Keys.NUMPAD3)
        if key == 'Enter':
            Functions.get_elements(self, entity).send_keys(Keys.ENTER)
        if key == 'Tab':
            Functions.get_elements(self, entity).send_keys(Keys.TAB)
        if key == 'Space':
            Functions.get_elements(self, entity).send_keys(Keys.SPACE)
        if key == 'Ctrl':
    ...
    def write_number(self, entity, text):
        #self.driver.execute_script("arguments[0].value='"+ text + "';", entity)
        #print(text)
        #Functions.get_elements(self, entity).send_keys(text)
        #self.driver.execute_script("arguments[0].setAttribute('value', '" + text + "')", entity)

        self.driver.execute_script("arguments[0].click();", entity)
...
##########################################################################
 ...
 ##########################################################################
    ...
    ###############ASERT NOT NULL ######################
    def assert_notnull(self, locator):
   ...
   # def load_file(self, text):
        ...
        #      autoit.win_wait("[CLASS:#32770;TITLE:Abrir]", 60)
        #      time.sleep(3)
        #     autoit.control_send("[CLASS:#32770;TITLE:Abrir]", "Edit1", "publica_deudor2.csv")
